drop procedure if exists etl_hst_oh//

CREATE DEFINER=`dbadmin`@`localhost` PROCEDURE `etl_hst_oh`()
...
BEGIN
        ...
        SELECT @etl_imp_id, @client_id, 'etl_hst_oh', 'b', v_etl_rpt_flag, v_etl_bm_build_flag;
        ...
        IF  @upload_id > 1
            THEN 
                SELECT 'New OH oaa/ogt record' AS Uploader_OH_ogt_oaa, convert_tz(now(), 'UTC', 'US/Eastern') AS Begin_Time;
                    INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
                    SELECT @etl_imp_id, @client_id, 'etl_hst_load_oh_oaa_student_plus()', 'b', v_etl_rpt_flag, v_etl_bm_build_flag;
                    call etl_hst_load_oh_ogt_student_plus();
                    call etl_hst_load_oh_oaa_student_plus();
                    ...
                    Insert into tmp_test_type(test_type_moniker)
                         select distinct tt.moniker
                          ...
                          where sub.ayp_subject_code in ('oaaMath','oaaRead','oaaScience',
															                           'oagtMath','ogtRead','ogtScience') 
					          ; 
                    
                    
                    SET v_etl_rpt_flag = 1;
                    SET @sql_OH_ogt_oaa_log := '';
                    SET @sql_OH_ogt_oaa_log := CONCAT('call ', @db, '.imp_set_upload_file_status (\'', 'etl_hst_load_oh_oaa_student_plus', '\',', '\'', 'P', '\',', '\'', 'ETL Load Successful', '\')');
                        prepare sql_OH_ogt_oaa_log from @sql_OH_ogt_oaa_log;
                                
                        execute sql_OH_ogt_oaa_log;
                            
                        deallocate prepare sql_OH_ogt_oaa_log;   
                    INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
                    SELECT @etl_imp_id, @client_id, 'etl_hst_load_oh_oaa_student_plus()', 'c', v_etl_rpt_flag, v_etl_bm_build_flag;
                SELECT 'New OH oaa/ogt  record' AS Uploader_OH_ogt_oaa, convert_tz(now(), 'UTC', 'US/Eastern') AS End_Time;
            ELSE SELECT 'No new OH oaa/ogt record' AS Uploader_OH_ogt_oaa;
        END IF;
        ...
        INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
        SELECT @etl_imp_id, @client_id, 'etl_hst_oh', 'c', v_etl_rpt_flag, v_etl_bm_build_flag;
