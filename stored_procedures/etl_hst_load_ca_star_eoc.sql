...
drop procedure if exists etl_hst_load_ca_star_eoc//

create definer=`dbadmin`@`localhost` procedure etl_hst_load_ca_star_eoc()
contains sql
...
proc: begin
    ...
    declare v_star_test_type_eoc varchar(2);
    ...
    # EOC Records are designated by test type = '01' - among other things - see tmp_stu_admin logic
    set v_star_test_type_eoc := '01';
    ...
    if v_view_exists > 0 then
        ...
        where ods.star_test_type = v_star_test_type_eoc
        ...
        # Pass 1:  Get students for eoc Math subjects.  As designated by math_test_type between 1 and 8.  This is in subject xref
        #  Note: For time being, had to hard code the subject codes.  Probably want to model for this at some point.
        select  ods.row_num
        ...
        join    tmp_subject_list as sub
                on      ods.math_test_type = sub.client_ayp_subject_code
                and     sub.ayp_subject_code in('caeocGenMath', 'caeocSummMath', 'caeocAlgebraI', 'caeocIntegMath1', 'caeocGeometry', 'caeocIntegMath2', 'caeocAlgebraII', 'caeocIntegMath3')
        ...
        where   (ods.state_student_code is not null or ods.sis_student_code is not null)
          and   ods.star_test_type = v_star_test_type_eoc
        #and     ods.scale_score REGEXP '^[0-9]' > 0
        union all
        # Pass 2:  Get students for eoc Science subjects.  As designated by eoc_science_test_type between 1 and 8.  This is in subject xref
        #  Note: For time being, had to hard code the subject codes.  Probably want to model for this at some point.
        select  ods2.row_num
        ...
        join    tmp_subject_list as sub2
                on      ods2.eoc_science_test_type = sub2.client_ayp_subject_code
                and     sub2.ayp_subject_code in('caeocEarthScience', 'caeocBiology', 'caeocChemistry', 'caeocPhysics', 'caeocIntSci1', 'caeocIntSci2', ' caeocIntSci3', 'caeocIntSci4')
        ...
        where   (ods2.state_student_code is not null or ods2.sis_student_code is not null)
          and   ods2.star_test_type = v_star_test_type_eoc
        union all
        # Pass 3:  Get students for eoc World Histor subjects.  As designated by world_history_ss being present and valid.
        select  ods3.row_num
        ...
        cross join    c_ayp_subject as sub3
                on      sub3.ayp_subject_code = 'caeocWorldHist'
        ...
        where   (ods3.state_student_code is not null or ods3.sis_student_code is not null)
          and   ods3.star_test_type = v_star_test_type_eoc
          and   ods3.world_hist_ss REGEXP '^[0-9]' > 0   
        ...
        into v_ayp_subject_id, v_ayp_subject_code, v_ayp_strand_id, v_ayp_strand_code,  v_column_pe, v_column_ss;
