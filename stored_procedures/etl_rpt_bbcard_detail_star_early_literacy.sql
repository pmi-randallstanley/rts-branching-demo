DROP PROCEDURE IF EXISTS etl_rpt_bbcard_detail_star_early_literacy //

CREATE DEFINER=`dbadmin`@`localhost` PROCEDURE `etl_rpt_bbcard_detail_star_early_literacy`()
...
proc: begin
    ...
    declare v_test_code varchar(75) default 'starEarlyLiteracy';

    call set_db_vars(@client_id, @state_id, @db_name, @db_name_core, @db_name_ods, @db_name_ib, @db_name_view, @db_name_pend, @db_name_dw);

    set v_ods_table = 'pmi_ods_star_early_literacy';
    ...
    if v_view_exists > 0 then
        ...
        select mi.bb_measure_item_code
            ...
            ,case when mi.bb_measure_item_code like 'starELScaleScore%' then 'SS'
                    when mi.bb_measure_item_code like 'starELGradeEquiv%' then 'GP'
                    when mi.bb_measure_item_code like 'starELGenReadiness%' then 'GR'
                    when mi.bb_measure_item_code like 'starELGraphKnow%' then 'GK'
                    when mi.bb_measure_item_code like 'starELPhonAware%' then 'PA'
                    when mi.bb_measure_item_code like 'starELPhonics%' then 'PH'
        ...
        from (
              select m.bb_group_id
                    ...
                    ,case when x.ods_column = 'SS' then sa.SS
                        ...
                        when x.ods_column = 'GR' then sa.GR
                        when x.ods_column = 'GK' then sa.GK
                        when x.ods_column = 'PA' then sa.PA
                        when x.ods_column = 'PH' then sa.PH
