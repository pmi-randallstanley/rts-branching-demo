...
$HeadURL: http://atlanta-web.performancematters.com:8099/svn/pminternal/Data/Redwood/Core/stored_procedures/etl_hst_load_fl_fcat_crosspoint_science.sql $
$Id: etl_hst_load_fl_fcat_crosspoint_science.sql 8518 2010-05-06 12:02:08Z mike.torian $ 
*/

drop procedure if exists etl_hst_load_fl_fcat_crosspoint_science//

create definer=`dbadmin`@`localhost` procedure etl_hst_load_fl_fcat_crosspoint_science()
contains sql
...
proc: begin 
    ...
    set v_ods_table := 'pmi_ods_ltdb_crosspoint_science';
    set v_ods_view := 'v_pmi_ods_ltdb_crosspoint_science';
    ...
    if v_view_exists > 0 then
        ...
        ;
            ...
            insert into tmp_fcat_date_conversion (
                    ...
                    ,day_id)
            select 
                    str_to_date(concat(mm,dd,yy),'%m%d%Y') as test_date
                    ,yy as year_id
                    ,mm as month_id_
                    ,dd as day_id
            from v_pmi_ods_ltdb_crosspoint_science
            where   yy is not null 
                    and mm is not null
            group by str_to_date(concat(mm,dd,yy),'%m%d%Y');
            ...
            ('fcat','02', 'fcatscience', 'fcatSciencePC')
            ,('fcat','03', 'fcatscience', 'fcatScienceES')
            ,('fcat','04', 'fcatscience', 'fcatScienceLE')
            ,('fcat','05', 'fcatscience', 'fcatScienceST')
            ;
