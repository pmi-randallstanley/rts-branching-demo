...
$HeadURL: http://atlanta-web.performancematters.com:8099/svn/pminternal/Data/Redwood/Core/stored_procedures/etl_bm_load_fl_manatee.sql $
$Id: etl_bm_load_fl_manatee.sql 8553 2010-05-10 15:21:53Z randall.stanley $ 
 */

DROP PROCEDURE IF EXISTS etl_bm_load_fl_manatee //

create definer=`dbadmin`@`localhost` procedure etl_bm_load_fl_manatee()
CONTAINS SQL
...
##################################
    ...
    UPDATE sam_test_event te
        JOIN    sam_test t
            ON      te.test_id = t.test_id SET
    te.import_xref_code = t.moniker
    WHERE te.import_xref_code IS NULL;
...
####################################
    ...
    SET @sql_sam_test := CONCAT(@sql_sam_test, ' SELECT DISTINCT pmi_f_get_next_sequence_app_db(\'sam_test\', 1), at.test_id ,COALESCE(at.test_name, at.test_id), \'e\', 70, 50, 1, ', @client_id, ', ', @client_id, ', 1234, 0');
    SET @sql_sam_test := CONCAT(@sql_sam_test, ' FROM ', @db_name_ods, '.pmi_ods_scan_results_fl_manatee at ');
    SET @sql_sam_test := CONCAT(@sql_sam_test, ' WHERE NOT EXISTS (SELECT * FROM sam_test t WHERE at.test_id = t.import_xref_code) AND at.test_id IS NOT null');
    ...
    SET @sql_sam_test_event := CONCAT(@sql_sam_test_event, ' FROM ', @db_name_ods, '.pmi_ods_scan_results_fl_manatee at');
...
##########################
    ...
    SET @sql_text_pivot := CONCAT('call ', @db_name_ods, '.etl_scan_results_fl_manatee_pivot()');
    ...
    # use work table here as a work-around for MySQL trigger bug
    SET @sql_sam_answer_key := '';
    SET @sql_sam_answer_key := CONCAT(@sql_sam_answer_key, ' CREATE TABLE tmp_ak_insert ');
    SET @sql_sam_answer_key := CONCAT(@sql_sam_answer_key, ' SELECT DISTINCT t.test_id, pmi_f_get_next_sequence_app_db(\'sam_answer_key\', 1) as test_question_id, at.test_id as import_xref_code, ');
    SET @sql_sam_answer_key := CONCAT(@sql_sam_answer_key, '    1 as section_num, at.question_eid as section_question_num, 1000001 as question_type_id, 1000002 as rubric_id, ');
    SET @sql_sam_answer_key := CONCAT(@sql_sam_answer_key, '    CONCAT(\'1-\',at.question_eid) as question_label, at.question_eid as flatfile_question_num ');
    ...
    INSERT  sam_answer_key (test_id, test_question_id, import_xref_code, section_num, section_question_num, question_type_id, rubric_id, question_label, flatfile_question_num, last_user_id)
    SELECT  test_id, test_question_id, import_xref_code, section_num, section_question_num, question_type_id, rubric_id, question_label, flatfile_question_num
    FROM    tmp_ak_insert
    ;
...
#############################################
    ...
    SET @sql_text_sam_test_student := CONCAT(@sql_text_sam_test_student, 'FROM ', @db_name_ods, '.pmi_ods_scan_results_fl_manatee AS sr ');
    ...
    SET @sql_text_sam_test_student := CONCAT(@sql_text_sam_test_student, 'JOIN c_student AS s ON s.student_code = right(sr.Student_ID,10) ');
...
#############################################
    ...
    UPDATE  sam_student_response as er
        JOIN    tmp_score_cr as tmp
            ON      tmp.test_id = er.test_id
            AND     tmp.test_question_id = er.test_question_id
            AND     tmp.rubric_score = coalesce((er.student_answer * 1), '0')
...
#################
        ...
        SET @sql_scan_log := '';
        SET @sql_scan_log := CONCAT('call ', @db_name_ods, '.imp_set_upload_file_status (\'', 'pmi_ods_scan_results_fl_manatee', '\',', '\'', 'P', '\',', '\'', 'ETL Load Successful', '\')');
