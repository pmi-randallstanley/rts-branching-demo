...
drop procedure if exists etl_hst_load_ga_ghsgt_math_qcc_gps//


create definer=`dbadmin`@`localhost` procedure etl_hst_load_ga_ghsgt_math_qcc_gps()
contains sql
...
proc: begin 
    ...
    if v_view_exists > 0 then
        ...
        where sub.ayp_subject_code IN('ghsgtMath','ghsgtGPSMath')
        ;  
        ...
        select  ods.row_num
            ,ods.stuid
            ,case when ods.matver = 'T' then 'Q' else ods.matver end matver 
            ,ods.testmo
            ,ods.testyr
            ,s.student_id
        ...
        join tmp_ghsgt_subject_list sub
            ...
            and  sub.ayp_subject_code = 'ghsgtMath'
