...
drop procedure if exists etl_hst_load_ca_cahsee//

create definer=`dbadmin`@`localhost` procedure etl_hst_load_ca_cahsee()
contains sql
...
proc: begin
    ...
    declare v_column_strand_pct varchar(50);
    ...
    set v_ods_table := 'pmi_ods_ca_cahsee';
    set v_ods_view := 'v_pmi_ods_ca_cahsee';
    ...
    if v_view_exists > 0 then
        ...
        create table `tmp_strand_score_moniker` (
                ...
                `column_strand_pct` varchar(50) not null,
                `column_ss` varchar(50) not null,
        ...
        select  ods.math_test_date as ods_test_date
            ,max(str_to_date(ods.math_test_date, '%m%d%Y')) as test_date
        from v_pmi_ods_ca_cahsee as ods
        group by    ods.math_test_date
        union #  Not All - would return duplicates
        select  ods.ela_test_date as ods_test_date
            ,max(str_to_date(ods.ela_test_date, '%m%d%Y')) as test_date
        from v_pmi_ods_ca_cahsee as ods
        group by    ods.ela_test_date
        ;
        ...
        select  ayps.ayp_subject_id
                ...
                ,min(score.moniker) as column_strand_pct
                ,min(sublist.column_ss) as column_ss
        from v_imp_table_column_ayp_strand as ayps
        join v_imp_table as tab
                on ayps.table_id = tab.table_id
        join v_imp_table_column as score 
                on  ayps.table_id = score.table_id
                and ayps.score_column_id = score.column_id
        ...
        # Pass 1:  Get students for CAHSEE Math
        #### NOTE:  Only matches to sis student code.  This is how the test data is, but will prob want to check all id''s later.
        select  ods.row_num
            ,ods.student_id
            ,ods.math_test_date
            ,s.student_id
        ...
        join    tmp_test_date dt
                on      ods.math_test_date = dt.ods_test_date
        join    c_student as s
        ...
        cross join    c_ayp_subject as sub
                on      sub.ayp_subject_code = 'cahseeMath'
        ...
        union all
        # Pass 2:  Get students for CAHSEE English
        #### NOTE:  Only matches to sis student code.  This is how the test data is, but will prob want to check all id''s later.
        select  ods.row_num
            ,ods.student_id
            ,ods.ela_test_date
        ...
        from    v_pmi_ods_ca_cahsee as ods
        join    tmp_test_date dt
                on      ods.ela_test_date = dt.ods_test_date
        join    c_student as s
                on      s.student_code = ods.student_id
        join    c_school_year as sy
                on     dt.test_date between sy.begin_date and sy.end_date
        cross join    c_ayp_subject as sub
                on      sub.ayp_subject_code = 'cahseeEnglish'
        left join    c_student_year as syr
                on syr.student_id = s.student_id
                and syr.school_year_id = sy.school_year_id
        where   ods.student_id is not null
          and   ods.ela_ss REGEXP '^[0-9]' > 0 
        ...
        into v_ayp_subject_id, v_ayp_subject_code, v_ayp_strand_id, v_ayp_strand_code,  v_column_strand_pct, v_column_ss;
            ...
            SET @sql_text := concat(@sql_text,' insert c_ayp_strand_student (ayp_subject_id, ayp_strand_id, student_id, school_year_id '
                                             ...
                                             ,'         ,sadmin.school_year_id '
                                             ,'         ,sadmin.test_month '
                                             ,'         ,cast(ods.', v_column_strand_pct, ' as decimal(9,3))  as ayp_score '
                                             ,'         ,null '
                                             ,'         ,null '
                                             ...
                                             ,'     and sadmin.ayp_subject_id = ', v_ayp_subject_id
                                             ,' where ods.', v_column_strand_pct, ' REGEXP \'^[0-9]\' > 0 '   
