drop procedure if exists etl_hst_nc//

CREATE DEFINER=`dbadmin`@`localhost` PROCEDURE `etl_hst_nc`()
...
BEGIN
        ...
        SELECT @etl_imp_id, @client_id, 'etl_hst_nc', 'b', v_etl_rpt_flag, v_etl_bm_build_flag;
        ...
        call etl_imp_get_queued_id_by_table_name(@upload_id, 'pmi_ods_nc_eoc_eog_ltdb');
    
        IF  @upload_id > 1
            THEN 
                SELECT 'New nc eoc eog record' AS Uploader_nc_eoc_eog, convert_tz(now(), 'UTC', 'US/Eastern') AS Begin_Time;
                    INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
                    SELECT @etl_imp_id, @client_id, 'etl_hst_load_nc_eoc_eog()', 'b', v_etl_rpt_flag, v_etl_bm_build_flag;

                    call etl_hst_load_nc_eoc_eog_ltdb();
                    ...
                    Insert into tmp_test_type(test_type_moniker)
                         select distinct tt.moniker
                          ...
                          where sub.ayp_subject_code in ('nceocAlgebraI','nceocBiology','nceogELA','nceogMath')                      
                    ;                    
                    
                    
                    SET v_etl_rpt_flag = 1;
                    SET @sql_nc_eoc_eog_log := '';
                    SET @sql_nc_eoc_eog_log := CONCAT('call ', @db, '.imp_set_upload_file_status (\'', 'etl_hst_load_nc_eoc_eog', '\',', '\'', 'P', '\',', '\'', 'ETL Load Successful', '\')');
                        prepare sql_nc_eoc_eog_log from @sql_nc_eoc_eog_log;
                                
                        execute sql_nc_eoc_eog_log;
                            
                        deallocate prepare sql_nc_eoc_eog_log;   
                    INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
                    SELECT @etl_imp_id, @client_id, 'etl_hst_load_nc_eoc_eog()', 'c', v_etl_rpt_flag, v_etl_bm_build_flag;
                SELECT 'New nc eoc eog record' AS Uploader_nc_eoc_eog, convert_tz(now(), 'UTC', 'US/Eastern') AS End_Time;
            ELSE SELECT 'No new nc eoc eog record' AS Uploader_nc_eoc_eog;
        END IF;
        ...
        INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
        SELECT @etl_imp_id, @client_id, 'etl_hst_nc', 'c', v_etl_rpt_flag, v_etl_bm_build_flag;
