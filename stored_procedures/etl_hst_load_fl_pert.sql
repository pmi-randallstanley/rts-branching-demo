drop procedure if exists etl_hst_load_fl_pert//

create definer=`dbadmin`@`localhost` procedure etl_hst_load_fl_pert()
contains sql
...
proc: begin 
    ...
    Declare v_ayp_subject_id_pert_math  int(11);
    Declare v_ayp_subject_id_pert_reading  int(11);
    Declare v_ayp_subject_id_pert_writing  int(11);
 
            
    call set_db_vars(@client_id, @state_id, @db_name, @db_name_core, @db_name_ods, @db_name_ib, @db_name_view, @db_name_pend, @db_name_dw);

    set v_ods_table = 'pmi_ods_fl_pert';
    set v_ods_view = 'v_pmi_ods_fl_pert';
    ...
    if v_view_exists > 0 then
        ...
        from    v_pmi_ods_fl_pert as ods
        group by    ods.test_mo_year
        ;
        
        
        ## Get Subject Variables
        select  min(case when ayp_subject_code = 'flpertMath' then ayp_subject_id end)
               ,min(case when ayp_subject_code = 'flpertReading' then ayp_subject_id end)
               ,min(case when ayp_subject_code = 'flpertWriting' then ayp_subject_id end)
        into    v_ayp_subject_id_pert_math
               ,v_ayp_subject_id_pert_reading
               ,v_ayp_subject_id_pert_writing

        from    c_ayp_subject as sub
        where   ayp_subject_code in ('flpertMath','flpertReading','flpertWriting')
        ;
        ...
        join    c_ayp_subject sub
                ...
                and     sub.ayp_subject_code = 'flpertMath'
        ...
        join    c_ayp_subject sub
                on      tsa.ayp_subject_id = sub.ayp_subject_id
                and     sub.ayp_subject_code = 'flpertReading'
        on duplicate key update last_user_id = values(last_user_id)
            ,ayp_score = values(ayp_score)
        ;
        ...
        join    c_ayp_subject sub
                on      tsa.ayp_subject_id = sub.ayp_subject_id
                and     sub.ayp_subject_code = 'flpertWriting'
        on duplicate key update last_user_id = values(last_user_id)
            ,ayp_score = values(ayp_score)
        ;
