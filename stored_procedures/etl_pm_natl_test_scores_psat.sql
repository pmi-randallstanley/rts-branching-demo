...
$HeadURL: http://atlanta-web.performancematters.com:8099/svn/pminternal/Data/Redwood/Core/stored_procedures/etl_pm_natl_test_scores_psat.sql $
$Id: etl_pm_natl_test_scores_psat.sql 7435 2009-07-24 13:58:49Z randall.stanley $ 
 */

####################################################################
# Insert PSAT data into pm_natl_test_scores.
# 
####################################################################

DROP PROCEDURE IF EXISTS etl_pm_natl_test_scores_psat//

CREATE definer=`dbadmin`@`localhost` procedure etl_pm_natl_test_scores_psat()
CONTAINS SQL
...
PROC: BEGIN 
    ...
    and     t.table_name = 'v_pmi_ods_psat';

    if @view_exists > 0 then

        set @psatTestDateFormatMask := pmi_f_get_etl_setting('psatTestDateFormatMask');

        if @psatTestDateFormatMask is not null then
            set v_date_format_mask = @psatTestDateFormatMask;
        end if;
        ...
        SET @sql_scan_log := CONCAT('call ', @db_name_ods, '.imp_set_upload_file_status (\'', 'pmi_ods_psat', '\',', '\'', 'P', '\',', '\'', 'ETL Load Successful', '\')');
