...
drop procedure if exists etl_imp_attend_type//

create definer=`dbadmin`@`localhost` procedure etl_imp_attend_type()
contains sql
...
proc: begin 
    ...
    set v_ods_table = 'pmi_ods_attend_type';
    ...
    if v_view_exists > 0 then
        ...
        select  pmi_f_get_next_sequence_app_db('c_attendance_type', 1), ods.cat_code
        from    v_pmi_ods_attend_type as ods
        left join   c_attendance_type as tar
                on      ods.cat_code = tar.cat_code
        where   tar.cat_id is null
        ;

        insert c_attendance_type (
            type_id
            ,type_code
            ,moniker
            ,description
