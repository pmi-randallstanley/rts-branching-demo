drop procedure if exists etl_color_fountas_pinnell//

create definer=`dbadmin`@`localhost` procedure etl_color_fountas_pinnell()
contains sql
...
proc: begin 
    ...
    set v_ods_table = 'pmi_ods_color_fountas_pinnell';
    ...
    if v_view_exists > 0 then
        ...
        select  count(*) 
        into    @view_count
        from    v_pmi_ods_color_fountas_pinnell;
               
        if @view_count > 0 then

            truncate TABLE pm_color_fountas_pinnell;
            
            INSERT pm_color_fountas_pinnell (bb_group_id, bb_measure_id, bb_measure_item_id, begin_year, end_year, begin_grade_sequence, end_grade_sequence, color_id, reading_level, last_user_id, create_timestamp, last_edit_timestamp)
            SELECT  g.bb_group_id
                   ...
                   ,now()
                   ,now()  
            FROM v_pmi_ods_color_fountas_pinnell ods
            ...
            JOIN  pm_bbcard_group g
              ...
              and g.bb_group_code = 'fountasPinnell' 
            JOIN  pmi_color c
            ...
            ON DUPLICATE key UPDATE last_user_id = 1234, reading_level = ods.reading_level
            ;
            ...
            join    c_color_swatch as cs
                    ...
                    and     cs.swatch_code = 'fountasPinnell'
            ;
            ...
            from    (
                        ...
                        from    pmi_color as c
                        join    pm_color_fountas_pinnell as csrc
                                on      c.color_id = csrc.color_id
                        join  c_color_swatch as cs
                                on      cs.swatch_code = 'fountasPinnell'
