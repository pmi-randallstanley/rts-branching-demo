...
$HeadURL: http://atlanta-web.performancematters.com:8099/svn/pminternal/Data/Redwood/Core/stored_procedures/etl_hst_load_md_hsa_student_plus.sql $
...
proc: begin 
   ...
   if v_view_exists > 0 then
        ...
        drop table if exists `tmp_hsa_subject_xref`;
        drop table if exists `tmp_hsa_strand_xref`;
        drop table if exists `tmp_hsa_stu_admin`;
        ...
        CREATE TABLE `tmp_hsa_subject_xref` (
          ...
          UNIQUE KEY `uq_tmp_hsa_subject_xref` (`client_ayp_subject_code`)
        ) ENGINE=InnoDB DEFAULT CHARSET=latin1
        ;

        CREATE TABLE `tmp_hsa_strand_xref` (
          `ayp_subject_id` int(10) NOT NULL,
        ...
        CREATE TABLE `tmp_hsa_stu_admin` (
        ...
        where   tt.moniker = 'hsa'
        ;

        
        # Strand Xref
        insert  tmp_hsa_strand_xref (
                  ayp_subject_id
                  ,ayp_strand_id
       ...
       ;
        ...
        if v_backfill_needed > 0 then
            ...
            from    tmp_hsa_stu_admin as sadmin
        ...
        select  tsa.ayp_subject_id
                ...
                ,now()
        from tmp_hsa_stu_admin as tsa
        join  tmp_hsa_strand_xref as hsl
        ...
        select  count(*)
        into    v_delete_count
        from    tmp_hsa_stu_admin as tmp1
