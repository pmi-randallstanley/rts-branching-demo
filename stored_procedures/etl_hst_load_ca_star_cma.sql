...
drop procedure if exists etl_hst_load_ca_star_cma//

create definer=`dbadmin`@`localhost` procedure etl_hst_load_ca_star_cma()
contains sql
...
proc: begin
    ...
    declare v_star_test_type_cma varchar(2);
    ...
    # CMA Records are designated by test type = '01' - among other things - see tmp_stu_admin logic
    set v_star_test_type_cma := '01';
    ...
    if v_view_exists > 0 then
        ...
        where ods.star_test_type = v_star_test_type_cma
        ...
        # Pass 1:  Get students for cmaELA.  As designated by cst_cma_ela_flag = 2 - which is specified in subject xref table
        select  ods.row_num
        ...
        join    tmp_subject_list as sub
                on      ods.cst_cma_ela_flag = sub.client_ayp_subject_code
                and     sub.ayp_subject_code = 'cmaELA'
        ...
        where   (ods.state_student_code is not null or ods.sis_student_code is not null)
          and   ods.star_test_type = v_star_test_type_cma
        #and     ods.scale_score REGEXP '^[0-9]' > 0
        union all
        # Pass 2:  Get students for cmaMath.  As designated by cst_cma_math_flag = 2 - which is specified in subject xref table
        select  ods2.row_num
        ...
        join    tmp_subject_list as sub2
                on      ods2.cst_cma_math_flag = sub2.client_ayp_subject_code
                and     sub2.ayp_subject_code = 'cmaMath'
        ...
        where   (ods2.state_student_code is not null or ods2.sis_student_code is not null)
          and   ods2.star_test_type = v_star_test_type_cma
        union all
        # Pass 3:  Get students for cmaScience.  As designated by cst_cma_science_flag = 2 - which is specified in subject xref table
        select  ods3.row_num
        ...
        join    tmp_subject_list as sub3
                on      ods3.cst_cma_science_flag = sub3.client_ayp_subject_code
                and     sub3.ayp_subject_code = 'cmaScience'
        ...
        where   (ods3.state_student_code is not null or ods3.sis_student_code is not null)
          and   ods3.star_test_type = v_star_test_type_cma
