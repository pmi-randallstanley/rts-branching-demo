DROP PROCEDURE IF EXISTS etl_rpt_baseball_detail_ga_g5wa//

CREATE definer=`dbadmin`@`localhost` procedure etl_rpt_baseball_detail_ga_g5wa()
...
$HeadURL: http://atlanta-web.performancematters.com:8099/svn/pminternal/Data/Redwood/Core/stored_procedures/etl_rpt_baseball_detail_ga_g5wa.sql $
$Id: etl_rpt_baseball_detail_ga_g5wa.sql 8352 2010-03-26 17:55:02Z mike.torian $ 
*/
...
BEGIN
    ...
    AND     t.table_name = 'v_pmi_ods_ga_g5wa';
    ...
    IF v_view_exists > 0 THEN
        ...
        WHERE  school_year_id = v_school_year_id AND 
                bb_group_id IN (SELECT bb_group_id FROM pm_baseball_group WHERE bb_group_code = 'gag5wa');
       

####################################################################
# Insert GA Writing G5WA data into rpt tables for baseball report.
####################################################################
        ...
        SELECT bg.bb_group_id,
               ...
               sy.school_year_id,
               MAX(CASE 
                        WHEN bm.bb_measure_code = 'gag5waScaleScore'   THEN wrtss
                        WHEN bm.bb_measure_code = 'gag5waIdeas'        THEN dom1
                        WHEN bm.bb_measure_code = 'gag5waOrganization' THEN dom2
                        WHEN bm.bb_measure_code = 'gag5waStyle'        THEN dom3
                        WHEN bm.bb_measure_code = 'gag5waConventions'  THEN dom4
                   END) AS score,
               MAX(CASE 
                        when bm.bb_measure_code = 'gag5waScaleScore'   THEN clr.moniker 
                        ELSE NULL
                   END) AS score_color,
               1234,
               now()
        FROM  v_pmi_ods_ga_g5wa AS ts
        JOIN  tmp_stu_admin as tmp
                ON ts.stuid = tmp.student_code    
        JOIN  c_student AS s
                ON s.student_id = tmp.student_id
        JOIN  pm_baseball_group AS bg 
                ON bg.bb_group_code = 'gag5wa'
        ...
        SET @sql_scan_log := '';
        SET @sql_scan_log := CONCAT('call ', @db_name_ods, '.imp_set_upload_file_status (\'', 'pmi_ods_ga_g5wa', '\',', '\'', 'P', '\',', '\'', 'ETL Load Successful', '\')');
