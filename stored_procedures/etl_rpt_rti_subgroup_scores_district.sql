drop procedure if exists etl_rpt_rti_subgroup_scores_district//

create definer=`dbadmin`@`localhost` procedure etl_rpt_rti_subgroup_scores_district()
contains sql
...
proc: begin 
    ...
    truncate table rpt_rti_subgroup_scores_district;
    
    insert rpt_rti_subgroup_scores_district (
        measure_id
        ,intervention_id
        ,event_date
        ,ayp_group_id
    ...
    group by si.measure_id, si.intervention_id, sipml.event_date, ag.ayp_group_id
    ;
