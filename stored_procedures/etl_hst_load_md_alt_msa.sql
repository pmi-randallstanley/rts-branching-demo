...
drop procedure if exists etl_hst_load_md_alt_msa// 
 
create definer=`dbadmin`@`localhost` procedure etl_hst_load_md_alt_msa()
contains sql
...
proc: begin 
    ...
    set v_ods_table = 'pmi_ods_md_alt_msa';
    set v_ods_view = 'v_pmi_ods_md_alt_msa';
    ...
    if v_view_exists > 0 then
            ...
            drop table if exists  `tmp_msa_date_conversion`;
            ...
            ;
             ...
             create table `tmp_msa_date_conversion` (
          `ods_test_date` varchar(10) not null,
          `test_month` tinyint(2) not null,
          `test_date` date not null,
          unique key `uq_tmp_msa_date_conversion` (`test_date`)
          ) engine=innodb default charset=latin1
          ;
      ...
      insert into tmp_msa_date_conversion (
            ods_test_date
            ,test_month
            ,test_date
        )
       ...
       select  ods.test_date as ods_test_date
            ,min(month(str_to_date(ods.test_date, '%Y%m%d'))) as test_month
            ,min(str_to_date(ods.test_date, '%Y%m%d')) as test_date
        from    v_pmi_ods_md_alt_msa as ods
        group by    ods.test_date;
            ...
            from    v_pmi_ods_md_alt_msa as ods
            join    c_student as s
                    on    s.student_code = ods.sis_student_code
            join    tmp_msa_date_conversion as dte
            ...
            where   ods.sis_student_code is not null
            union all
            select  ods2.row_num
                    ,ods2.sis_student_code
                    ,s2.student_id
                    ,ods2.test_date
            ...
            from    v_pmi_ods_md_alt_msa as ods2
            join    c_student as s2
                    on    s2.student_state_code = ods2.sis_student_code
            join    tmp_msa_date_conversion as dte2
            ...
            where   ods2.sis_student_code is not null
            union all
            select  ods3.row_num
                    ,ods3.sis_student_code
                    ,s3.student_id
                    ,ods3.test_date
            ...
            from    v_pmi_ods_md_alt_msa as ods3
            join    c_student as s3
                    on    s3.fid_code = ods3.sis_student_code
            join    tmp_msa_date_conversion as dte3
            ...
            where   ods3.sis_student_code is not null
        ...
        into v_ayp_subject_id, v_ayp_strand_id, v_column_ss;
