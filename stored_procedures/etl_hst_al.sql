drop procedure if exists etl_hst_al//

CREATE DEFINER=`dbadmin`@`localhost` PROCEDURE `etl_hst_al`()
...
BEGIN
        ...
        SELECT @etl_imp_id, @client_id, 'etl_hst_al', 'b', v_etl_rpt_flag, v_etl_bm_build_flag;
        ...
        call etl_imp_get_queued_id_by_table_name(@upload_id, 'pmi_ods_al_armt_math_reading');
    
        IF  @upload_id > 1
            THEN 
                SELECT 'New AL ARMT record' AS Uploader_al_ARMT, convert_tz(now(), 'UTC', 'US/Eastern') AS Begin_Time;
                    INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
                    SELECT @etl_imp_id, @client_id, 'etl_hst_load_al_ARMT()', 'b', v_etl_rpt_flag, v_etl_bm_build_flag;

                    call etl_hst_load_al_ARMT();
                    ...
                    Insert into tmp_test_type(test_type_moniker)
                         select distinct tt.moniker
                          ...
                          where sub.ayp_subject_code in ('alARMTMath','alARMTRead')                      
                    ;                    
                    
                    
                    SET v_etl_rpt_flag = 1;
                    SET @sql_al_ARMT_log := '';
                    SET @sql_al_ARMT_log := CONCAT('call ', @db, '.imp_set_upload_file_status (\'', 'etl_hst_load_al_ARMT', '\',', '\'', 'P', '\',', '\'', 'ETL Load Successful', '\')');
                        prepare sql_al_ARMT_log from @sql_al_ARMT_log;
                                
                        execute sql_al_ARMT_log;
                            
                        deallocate prepare sql_al_ARMT_log;   
                    INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
                    SELECT @etl_imp_id, @client_id, 'etl_hst_load_al_ARMT()', 'c', v_etl_rpt_flag, v_etl_bm_build_flag;
                SELECT 'New AL ARMT record' AS Uploader_al_ARMT, convert_tz(now(), 'UTC', 'US/Eastern') AS End_Time;
            ELSE SELECT 'No new AL ARMT record' AS Uploader_al_ARMT;
        END IF;
        ...
        INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
        SELECT @etl_imp_id, @client_id, 'etl_hst_al', 'c', v_etl_rpt_flag, v_etl_bm_build_flag;
