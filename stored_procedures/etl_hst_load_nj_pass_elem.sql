...
drop procedure if exists etl_hst_load_nj_pass_elem//

create definer=`dbadmin`@`localhost` procedure etl_hst_load_nj_pass_elem()
...
proc: begin 
   ...
   select  count(*)
    ...
    if v_view_exists > 0 and v_strand_grade_table_exists > 0 then
        ...
        into v_ayp_subject_id, v_ayp_strand_id, v_column_ss,v_grade_code;
