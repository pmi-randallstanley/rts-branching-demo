drop procedure if exists etl_hst_co//

CREATE DEFINER=`dbadmin`@`localhost` PROCEDURE `etl_hst_co`()
...
BEGIN
        ...
        SELECT @etl_imp_id, @client_id, 'etl_hst_co', 'b', v_etl_rpt_flag, v_etl_bm_build_flag;
        ...
        call etl_imp_get_queued_id_by_table_name(@upload_id, 'pmi_ods_co_tcap');
    
        IF  @upload_id > 1
            THEN 
                SELECT 'New CO TCAP record' AS Uploader_co_tcap, convert_tz(now(), 'UTC', 'US/Eastern') AS Begin_Time;
                    INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
                    SELECT @etl_imp_id, @client_id, 'etl_hst_load_co_tcap()', 'b', v_etl_rpt_flag, v_etl_bm_build_flag;

                    call etl_hst_load_co_tcap();
                    ...
                    Insert into tmp_test_type(test_type_moniker)
                         select distinct tt.moniker
                          ...
                          where sub.ayp_subject_code like 'tcap%'                     
                    ;                    
                    
                    
                    SET v_etl_rpt_flag = 1;
                    SET @sql_co_tcap_log := '';
                    SET @sql_co_tcap_log := CONCAT('call ', @db, '.imp_set_upload_file_status (\'', 'etl_hst_load_co_tcap', '\',', '\'', 'P', '\',', '\'', 'ETL Load Successful', '\')');
                        prepare sql_co_tcap_log from @sql_co_tcap_log;
                                
                        execute sql_co_tcap_log;
                            
                        deallocate prepare sql_co_tcap_log;   
                    INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
                    SELECT @etl_imp_id, @client_id, 'etl_hst_load_co_tcap()', 'c', v_etl_rpt_flag, v_etl_bm_build_flag;
                SELECT 'New CO TCAP record' AS Uploader_co_tcap, convert_tz(now(), 'UTC', 'US/Eastern') AS End_Time;
            ELSE SELECT 'No New CO TCAP record' AS Uploader_co_tcap;
        END IF;
        ...
        INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
        SELECT @etl_imp_id, @client_id, 'etl_hst_co', 'c', v_etl_rpt_flag, v_etl_bm_build_flag;
