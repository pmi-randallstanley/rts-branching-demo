...
drop procedure if exists etl_hst_load_nj_spa//

create definer=`dbadmin`@`localhost` procedure etl_hst_load_nj_spa()
contains sql
...
proc: begin 
    ...
    set v_ods_table = 'pmi_ods_nj_spa';
    set v_ods_view = 'v_pmi_ods_nj_spa';
    ...
    if v_view_exists > 0 then
            ...
            drop table if exists `tmp_spa_date_conversion`;
            ...
            create table `tmp_spa_date_conversion` (
          `test_mo_year` varchar(10) not null,
          `test_month` tinyint(2) not null,
          `test_date` date not null,
          unique key `uq_tmp_spa_date_conversion` (`test_mo_year`)
          ) engine=innodb default charset=latin1
          ;
            ...
            insert into tmp_spa_date_conversion (
            test_mo_year
            ,test_month
            ,test_date
        )
        ...
        ;
            ...
            join    c_student as s
             ...
             join    tmp_spa_date_conversion as dte
            ...
            from    v_pmi_ods_nj_spa as ods3
            join    c_student as s3
                    on    s3.fid_code = ods3.state_student_code
            join    tmp_spa_date_conversion as dte3
