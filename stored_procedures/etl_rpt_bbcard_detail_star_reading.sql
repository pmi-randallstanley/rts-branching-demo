DROP PROCEDURE IF EXISTS etl_rpt_bbcard_detail_star_reading //

CREATE DEFINER=`dbadmin`@`localhost` PROCEDURE `etl_rpt_bbcard_detail_star_reading`()
...
proc: begin
    ...
    declare v_test_code varchar(75) default 'starReading';

    call set_db_vars(@client_id, @state_id, @db_name, @db_name_core, @db_name_ods, @db_name_ib, @db_name_view, @db_name_pend, @db_name_dw);

    set v_ods_table = 'pmi_ods_star_reading';
    ...
    if v_view_exists > 0 then
        ...
        create table `tmp_stu_admin` (
          ...
          `irl` varchar(20) default null,
          `lowerZPD` varchar(20) default null,
          `upperZPD` varchar(20) default null,
        ...
        CREATE TABLE `tmp_xref_table_column_measure_item` (
          ...
          `ods_column` varchar(10) character set utf8 default NULL
        ) ENGINE=InnoDB DEFAULT CHARSET=latin1
        ;
        ...
        from v_pmi_ods_star_reading ods;
        ...
        from    v_pmi_ods_star_reading ods2
        ...
        from    v_pmi_ods_star_reading ods3
        ...
        on duplicate key update ss = values(ss)
                                ...
                                ,nce = values(nce)
                                ,irl = values(irl)
                                ,upperZPD = values(upperZPD)
                                ,lowerZPD = values(lowerZPD)
        ...
        select mi.bb_measure_item_code
            ...
            ,case when mi.bb_measure_item_code like 'starReadGradePlace%' then 'GP'
                    when mi.bb_measure_item_code like 'starReadScaleScore%' then 'SS'
                    when mi.bb_measure_item_code like 'starReadGradeEquiv%' then 'GE'
                    when mi.bb_measure_item_code like 'starReadPercentRank%' then 'PR'
                    when mi.bb_measure_item_code like 'starReadNomalCrvEq%' then 'NCE'
                    when mi.bb_measure_item_code like 'starReadInstructLvl%' then 'IRL'
                    when mi.bb_measure_item_code like 'starReadLZonePrxDev%' then 'LowerZPD'
                    when mi.bb_measure_item_code like 'starReadUZonePrxDev%' then 'UpperZPD'
                    when mi.bb_measure_item_code like 'starReadOralReadFluency%' then 'ORF'
        ...
        where g.bb_group_code = 'star'
            and m.bb_measure_code = 'starReading'
        ;
        ...
        from (
              select m.bb_group_id
                    ...
                    ,case when x.ods_column = 'SS' then sa.SS
                        ...
                        when x.ods_column = 'IRL' then sa.IRL
                        when x.ods_column = 'LowerZPD' then sa.LowerZPD
                        when x.ods_column = 'UpperZPD' then sa.UpperZPD
                    end as score
