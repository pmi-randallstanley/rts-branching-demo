...
DROP PROCEDURE IF EXISTS etl_pm_natl_test_scores_psat_md_calvertnet//

CREATE definer=`dbadmin`@`localhost` procedure etl_pm_natl_test_scores_psat_md_calvertnet()
...
$HeadURL: http://atlanta-web.performancematters.com:8099/svn/pminternal/Data/Redwood/Core/stored_procedures/etl_pm_natl_test_scores_psat_md_calvertnet.sql $
$Id: etl_pm_natl_test_scores_psat_md_calvertnet.sql 7380 2009-07-16 14:23:58Z randall.stanley $ 
*/
...
BEGIN
    ...
    ####################################################################
    #
    # Insert PSAT data into pm_natl_test_scores for md_calvertnet
    # 
    ####################################################################
    ...
    if @view_exists > 0 then
        ...
        ##############################################################
        # Get md_calvertnet specific scores into standard psat view
        ##############################################################
    
         DROP TABLE IF EXISTS tmp_psat_md_calvertnet;
            
            CREATE TABLE tmp_psat_md_calvertnet
            SELECT  s.student_code AS student_id
                    ,STR_TO_DATE(test_date,  '%m/%d/%Y') AS test_date                         
                    ,CONCAT(p.score01,0) AS 'verbal_score'
                    ,CONCAT(p.score02,0) AS 'math_score'
                    ,CONCAT(p.score03,0) AS 'writing_score'                 
            FROM    v_pmi_ods_ltdb_pentamation p
                JOIN    c_student s
                    ON      s.student_code = p.student_id
           WHERE p.test_name like 'PSAT%';                 
                ...
                FROM (
                        SELECT 
                            ...
                            ,(CASE WHEN tsub.subject_code like '%Math'
                                    ...
                                    ELSE NULL
                                    END) AS score
                        FROM tmp_psat_md_calvertnet AS odsv
                        JOIN c_student st
                            ON st.student_code = odsv.student_id
                        JOIN pm_natl_test_type AS tty
                            ON  tty.test_type_code = 'psat'
            ...
            FROM (
                    ...
                    FROM 
                        ...
                        JOIN pm_natl_test_type AS tty
                            ...
                            AND  tty.test_type_code IN ('psat')
