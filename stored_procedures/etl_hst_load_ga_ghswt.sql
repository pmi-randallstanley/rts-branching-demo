...
drop procedure if exists etl_hst_load_ga_ghswt//

create definer=`dbadmin`@`localhost` procedure etl_hst_load_ga_ghswt()
contains sql
...
proc: begin 
    ...
    set v_ods_table = 'pmi_ods_ga_ghswt';
    set v_ods_view = 'v_pmi_ods_ga_ghswt';
    ...
    if v_view_exists > 0 then
            ...
            select  ods.row_num
                    ,ods.stuid
                    ,s.student_id
                    ,coalesce(ods.testmth,0) as month_id
                    ,YEAR(str_to_date(concat('20',ods.testyr),'%Y%m%d')) as school_year_id
                    ,ods.grade
                    ,ods.schcode
            ...
            left join c_student_year as sty
                    ...
                    and   sty.school_year_id = YEAR(str_to_date(concat('20',ods.testyr),'%Y%m%d'))
            where   ods.stuid is not null
            union all
            select  ods2.row_num
                    ,ods2.stuid
                    ,s2.student_id
                    ,coalesce(ods2.testmth,0) as month_id
                    ,YEAR(str_to_date(concat('20',ods2.testyr),'%Y%m%d')) as school_year_id
                    ,ods2.grade
                    ,ods2.schcode
            ...
            from    v_pmi_ods_ga_ghswt as ods2
            join    c_student as s2
                    on    s2.student_state_code = ods2.stuid
            left join c_student_year as sty2
                    on    sty2.student_id = s2.student_id 
                    and   sty2.school_year_id = YEAR(str_to_date(concat('20',ods2.testyr),'%Y%m%d'))
            where   ods2.stuid is not null
            union all
            select  ods3.row_num
                    ,ods3.stuid
                    ,s3.student_id
                    ,coalesce(ods3.testmth,0) as month_id
                    ,YEAR(str_to_date(concat('20',ods3.testyr),'%Y%m%d')) as school_year_id
                    ,ods3.grade
                    ,ods3.schcode
            ...
            from    v_pmi_ods_ga_ghswt as ods3
            join    c_student as s3
                    on    s3.fid_code = ods3.stuid
            left join c_student_year as sty3
                    on    sty3.student_id = s3.student_id 
                    and   sty3.school_year_id = YEAR(str_to_date(concat('20',ods3.testyr),'%Y%m%d'))
            where   ods3.stuid is not null
