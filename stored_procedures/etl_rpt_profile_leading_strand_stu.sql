...
$HeadURL: http://atlanta-web.performancematters.com:8099/svn/pminternal/Data/Redwood/Core/stored_procedures/etl_rpt_profile_leading_strand_stu.sql $
$Id: etl_rpt_profile_leading_strand_stu.sql 9366 2010-10-06 15:28:38Z randall.stanley $ 
 */

DROP PROCEDURE IF EXISTS etl_rpt_profile_leading_strand_stu //

CREATE definer=`dbadmin`@`localhost` procedure `etl_rpt_profile_leading_strand_stu`()
CONTAINS SQL
...
BEGIN
    ...
    TRUNCATE TABLE rpt_profile_leading_strand_stu;
    
    DROP TABLE IF EXISTS `tmp_rpt_lead_strand_stu_base`;

    CREATE TABLE `tmp_rpt_lead_strand_stu_base` (
      ...
      PRIMARY KEY  (`test_id`,`test_event_id`,`test_question_id`,`ayp_subject_id`,`ayp_strand_id`),
      KEY `ind_tmp_rpt_lead_strand_stu_base` (`ayp_subject_id`,`ayp_strand_id`,`sequence`)
    ) ENGINE=InnoDB DEFAULT CHARSET=latin1
    ;

    insert tmp_rpt_lead_strand_stu_base (
        test_id
        ,test_event_id
        ,test_question_id
        ,ayp_subject_id
    ...
    INSERT INTO rpt_profile_leading_strand_stu (
       ayp_subject_id
       ,ayp_strand_id
       ,student_id
       ,bm_01_pe
    ...
    FROM    (
                ...
                SELECT  tmp1.ayp_subject_id
                        ...
                        ,ROUND(sum(er.rubric_value), 1) as pe
                        ,sum(tmp1.rubric_total) as pp
                
                FROM    tmp_rpt_lead_strand_stu_base AS tmp1
    ...
    UPDATE   rpt_profile_leading_strand_stu AS  rplss
    JOIN     c_color_ayp_benchmark as cb
