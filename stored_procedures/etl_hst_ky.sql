drop procedure if exists etl_hst_ky//

CREATE DEFINER=`dbadmin`@`localhost` PROCEDURE `etl_hst_ky`()
...
BEGIN
        ...
        SELECT @etl_imp_id, @client_id, 'etl_hst_ky', 'b', v_etl_rpt_flag, v_etl_bm_build_flag;
        ...
        call etl_imp_get_queued_id_by_table_name(@upload_id, 'pmi_ods_ky_kcct');
    
        IF  @upload_id > 1
            THEN 
                SELECT 'New KY kcct record' AS Uploader_KY_kcct, convert_tz(now(), 'UTC', 'US/Eastern') AS Begin_Time;
                    INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
                    SELECT @etl_imp_id, @client_id, 'etl_hst_load_ky_kcct()', 'b', v_etl_rpt_flag, v_etl_bm_build_flag;

                    call etl_hst_load_ky_kcct();
                    ...
                    Insert into tmp_test_type(test_type_moniker)
                         select distinct tt.moniker
                          ...
                          where sub.ayp_subject_code in ('kcctMath','kcctReading','kcctScience',
														                             'kcctwriting','kcctAH','kcctPL')                     
                    ;                    
                    
                    
                    SET v_etl_rpt_flag = 1;
                    SET @sql_KY_kcct_log := '';
                    SET @sql_KY_kcct_log := CONCAT('call ', @db, '.imp_set_upload_file_status (\'', 'etl_hst_load_ky_kcct', '\',', '\'', 'P', '\',', '\'', 'ETL Load Successful', '\')');
                        prepare sql_KY_kcct_log from @sql_KY_kcct_log;
                                
                        execute sql_KY_kcct_log;
                            
                        deallocate prepare sql_KY_kcct_log;   
                    INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
                    SELECT @etl_imp_id, @client_id, 'etl_hst_load_ky_kcct()', 'c', v_etl_rpt_flag, v_etl_bm_build_flag;
                SELECT 'New KY kcct record' AS Uploader_KY_kcct, convert_tz(now(), 'UTC', 'US/Eastern') AS End_Time;
            ELSE SELECT 'No new KY kcct record' AS Uploader_KY_kcct;
        END IF;
        ...
        IF  @upload_id > 1
            THEN 
                SELECT 'New KY nwea record' AS Uploader_KY_nwea, convert_tz(now(), 'UTC', 'US/Eastern') AS Begin_Time;
                    INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
                    SELECT @etl_imp_id, @client_id, 'etl_hst_load_ky_nwea()', 'b', v_etl_rpt_flag, v_etl_bm_build_flag;

                    call etl_hst_load_ky_nwea();
                    ...
                    Insert into tmp_test_type(test_type_moniker)
                         select distinct tt.moniker
                           ...
                           join c_ayp_test_type tt
                             on sub.ayp_test_type_id = tt.ayp_test_type_id
                          where sub.ayp_subject_code in ('kyNWEAMath','kyNWEAReading')                  
                    ;                    
                    
                    
                    SET v_etl_rpt_flag = 1;
                    SET @sql_KY_nwea_log := '';
                    SET @sql_KY_nwea_log := CONCAT('call ', @db, '.imp_set_upload_file_status (\'', 'etl_hst_load_ky_nwea', '\',', '\'', 'P', '\',', '\'', 'ETL Load Successful', '\')');
                        prepare sql_KY_nwea_log from @sql_KY_nwea_log;
                                
                        execute sql_KY_nwea_log;
                            
                        deallocate prepare sql_KY_nwea_log;   
                    INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
                    SELECT @etl_imp_id, @client_id, 'etl_hst_load_ky_nwea()', 'c', v_etl_rpt_flag, v_etl_bm_build_flag;
                SELECT 'New KY nwea record' AS Uploader_KY_nwea, convert_tz(now(), 'UTC', 'US/Eastern') AS End_Time;
            ELSE SELECT 'No new KY nwea record' AS Uploader_KY_nwea;
        END IF;
        ...
        INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
        SELECT @etl_imp_id, @client_id, 'etl_hst_ky', 'c', v_etl_rpt_flag, v_etl_bm_build_flag;
