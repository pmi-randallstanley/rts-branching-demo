DROP PROCEDURE IF EXISTS etl_rpt_baseball_detail_ga_ghswt //

CREATE definer=`dbadmin`@`localhost` procedure etl_rpt_baseball_detail_ga_ghswt()
...
$HeadURL: http://atlanta-web.performancematters.com:8099/svn/pminternal/Data/Redwood/Core/stored_procedures/etl_rpt_baseball_detail_ga_ghswt.sql $
$Id: etl_rpt_baseball_detail_ga_ghswt.sql 8352 2010-03-26 17:55:02Z mike.torian $ 
*/

BEGIN

    declare v_school_year_id int(11);
    declare v_view_exists int(4);
    ...
    AND     t.table_name = 'v_pmi_ods_ga_ghswt';
    ...
    IF v_view_exists > 0 THEN
        ...
        WHERE  school_year_id = v_school_year_id 
               AND bb_group_id IN (SELECT bb_group_id FROM pm_baseball_group WHERE bb_group_code = 'gaghswt');
    
        ####################################################################
        # Insert GA Writing GHSWT data into rpt tables for baseball report.
        ####################################################################
        ...
        select  ods.row_num
                ...
                ,s.student_id
        from    v_pmi_ods_ga_ghswt as ods
        join    c_student as s
        ...
        SELECT bg.bb_group_id,
               ...
               sy.school_year_id,
               MAX(CASE 
                        WHEN bm.bb_measure_code = 'gaghswtScaleScore'   THEN wrtss
                        WHEN bm.bb_measure_code = 'gaghswtIdeas'        THEN dom1
                        WHEN bm.bb_measure_code = 'gaghswtOrganization' THEN dom2
                        WHEN bm.bb_measure_code = 'gaghswtStyle'        THEN dom3
                        WHEN bm.bb_measure_code = 'gaghswtConventions'  THEN dom4
                   END) AS score,
               MAX(case 
                        WHEN bm.bb_measure_code = 'gaghswtScaleScore'   THEN clr.moniker 
                        ELSE NULL
                   END) AS score_color,
               1234,
               now()
            FROM  v_pmi_ods_ga_ghswt AS ts
            JOIN  tmp_stu_admin as tmp
                ON ts.stuid = tmp.student_code
            JOIN  c_student AS s
                ON s.student_id = tmp.student_id
            JOIN  pm_baseball_group AS bg 
                ON bg.bb_group_code = 'gaghswt'
        ...
        SET @sql_scan_log := '';
        SET @sql_scan_log := CONCAT('call ', @db_name_ods, '.imp_set_upload_file_status (\'', 'pmi_ods_ga_ghswt', '\',', '\'', 'P', '\',', '\'', 'ETL Load Successful', '\')');
