...
drop procedure if exists etl_hst_load_md_msa_student_plus//

create definer=`dbadmin`@`localhost` procedure etl_hst_load_md_msa_student_plus()
contains sql
...
proc: begin 
    ...
    # Date Mask is consistent across existing customers that use Student Plus
    set v_ltdb_date_format_mask = '%Y-%m-%d %H:%i:%s.%f';
   ...
   if v_view_exists > 0 then
        ...
        drop table if exists `tmp_msa_subject_xref`;
        drop table if exists `tmp_msa_strand_xref`;
        drop table if exists `tmp_msa_stu_admin`;
        ...
        CREATE TABLE `tmp_msa_subject_xref` (
          `ayp_subject_id` int(10) NOT NULL,
          `ayp_subject_code` varchar(50) NOT NULL,
          `client_ayp_subject_code` varchar(25) NOT NULL,
          PRIMARY KEY  (`ayp_subject_id`,`client_ayp_subject_code`),
          UNIQUE KEY `uq_tmp_msa_subject_xref` (`client_ayp_subject_code`)
        ) ENGINE=InnoDB DEFAULT CHARSET=latin1
        ;

        CREATE TABLE `tmp_msa_strand_xref` (
          `ayp_subject_id` int(10) NOT NULL,
        ...
        CREATE TABLE `tmp_msa_stu_admin` (
        ...
        where   tt.moniker = 'msa'
        ;

        
        # Strand Xref
        insert  tmp_msa_strand_xref (
                  ayp_subject_id
                  ,ayp_strand_id
        ...
        values  ('SSCR', 'MD Calvertnet')
                ,('SS', 'MD SMCPS')
                ,('M_SS', 'MD SMCPS MSA Math')
                ,('R_SS', 'MD SMCPS MSA Reading')
        ;
      
                     
        #Temp Stu Admin - Load admin data by student, subject, year, month and score type
        insert  tmp_msa_stu_admin (
            student_code
            ,test_date
            ,row_num
            ,student_id
       ...
       select ods.student_id as student_code
           ,date(str_to_date(ods.test_date, v_ltdb_date_format_mask))
           ,ods.row_num
           ,s.student_id
           ,sub.ayp_subject_id
           ,month(str_to_date(ods.test_date, v_ltdb_date_format_mask)) as test_month
           ,ods.score_typ
           ,ods.score
           ,sy.school_year_id
           ,ods.grade
       ...
       join    tmp_msa_subject_xref as sub
       ...
       select ods2.student_id as student_code
           ,str_to_date(ods2.test_date, v_ltdb_date_format_mask)
           ,ods2.row_num
           ,s2.student_id
           ,sub2.ayp_subject_id
           ,month(str_to_date(ods2.test_date, v_ltdb_date_format_mask)) as test_month
           ,ods2.score_typ
           ,ods2.score
           ,sy2.school_year_id
           ,ods2.grade
       ...
       join    tmp_msa_subject_xref as sub2
       ...
       select ods3.student_id as student_code
           ,str_to_date(ods3.test_date, v_ltdb_date_format_mask)
           ,ods3.row_num
           ,s3.student_id
           ,sub3.ayp_subject_id
           ,month(str_to_date(ods3.test_date, v_ltdb_date_format_mask)) as test_month
           ,ods3.score_typ
           ,ods3.score
           ,sy3.school_year_id
           ,ods3.grade
       ...
       join    tmp_msa_subject_xref as sub3
       ...
       ;
        ...
        if v_backfill_needed > 0 then
            ...
            from    tmp_msa_stu_admin as sadmin
        ...
        insert c_ayp_strand_student (ayp_subject_id, ayp_strand_id ,student_id, school_year_id, month_id, ayp_score, score_type_code, last_user_id, create_timestamp)
        select  tsa.ayp_subject_id
                ...
                ,now()
        from tmp_msa_stu_admin as tsa
        join  tmp_msa_strand_xref as hsl
        ...
        select  count(*)
        into    v_delete_count
        from    tmp_msa_stu_admin as tmp1
