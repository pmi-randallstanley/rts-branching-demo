...
drop procedure if exists etl_hst_load_md_msa_math_reading//

create definer=`dbadmin`@`localhost` procedure etl_hst_load_md_msa_math_reading()
contains sql
...
proc: begin 
    ...
    set v_ods_table = 'pmi_ods_msa_math_reading';
    set v_ods_view = 'v_pmi_ods_msa_math_reading';
    ...
    if v_view_exists > 0 then
            ...
            select  ods.row_num
                    ,ods.pupil_number
                    ,s.student_id
                    ,coalesce(ods.test_mo,0) as month_id
                    ,ods.school_year
                    ,ods.grade
                    ,ods.school_number
            ...
            from    v_pmi_ods_msa_math_reading as ods
            join    c_student as s
                    on    s.student_code = ods.pupil_number
            left join c_student_year as sty 
                    on    sty.student_id = s.student_id 
                    and   sty.school_year_id = ods.school_year
            where   ods.pupil_number is not null
            union all
            select  ods2.row_num
                    ,ods2.pupil_number
                    ,s2.student_id
                    ,coalesce(ods2.test_mo,0) as month_id
                    ,ods2.school_year
                    ,ods2.grade
                    ,ods2.school_number
            ...
            from    v_pmi_ods_msa_math_reading as ods2
            join    c_student as s2
                    on    s2.student_state_code = ods2.pupil_number
            left join c_student_year as sty2
                    on    sty2.student_id = s2.student_id 
                    and   sty2.school_year_id = ods2.school_year
            where   ods2.pupil_number is not null
            union all
            select  ods3.row_num
                    ,ods3.pupil_number
                    ,s3.student_id
                    ,coalesce(ods3.test_mo,0) as month_id
                    ,ods3.school_year
                    ,ods3.grade
                    ,ods3.school_number
