drop procedure if exists etl_hst_ca//

CREATE DEFINER=`dbadmin`@`localhost` PROCEDURE `etl_hst_ca`()
...
BEGIN
        ...
        SELECT @etl_imp_id, @client_id, 'etl_hst_ca', 'b', v_etl_rpt_flag, v_etl_bm_build_flag;
        ...
        call etl_imp_get_queued_id_by_table_name(@upload_id, 'pmi_ods_ca_cahsee');
    
        IF  @upload_id > 1
            THEN 
                SELECT 'New ca cahsee record' AS Uploader_ca_cahsee, convert_tz(now(), 'UTC', 'US/Eastern') AS Begin_Time;
                    INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
                    SELECT @etl_imp_id, @client_id, 'etl_hst_load_ca_cahsee()', 'b', v_etl_rpt_flag, v_etl_bm_build_flag;

                    call etl_hst_load_ca_cahsee();
                    ...
                    Insert into tmp_test_type(test_type_moniker)
                         select distinct tt.moniker
                          ...
                          where sub.ayp_subject_code in ('cahseeEnglish','cahseeMath')                      
                    ;                    
                    
                    
                    SET v_etl_rpt_flag = 1;
                    SET @sql_ca_cahsee_log := '';
                    SET @sql_ca_cahsee_log := CONCAT('call ', @db, '.imp_set_upload_file_status (\'', 'etl_hst_load_ca_cahsee', '\',', '\'', 'P', '\',', '\'', 'ETL Load Successful', '\')');
                        prepare sql_ca_cahsee_log from @sql_ca_cahsee_log;
                                
                        execute sql_ca_cahsee_log;
                            
                        deallocate prepare sql_ca_cahsee_log;   
                    INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
                    SELECT @etl_imp_id, @client_id, 'etl_hst_load_ca_cahsee()', 'c', v_etl_rpt_flag, v_etl_bm_build_flag;
                SELECT 'New ca cahsee record' AS Uploader_ca_cahsee, convert_tz(now(), 'UTC', 'US/Eastern') AS End_Time;
            ELSE SELECT 'No new ca cahsee record' AS Uploader_ca_cahsee;
        END IF;
        
        call etl_imp_get_queued_id_by_table_name(@upload_id, 'pmi_ods_ca_star');
    
        IF  @upload_id > 1
            THEN 
                SELECT 'New ca star record' AS Uploader_ca_star, convert_tz(now(), 'UTC', 'US/Eastern') AS Begin_Time;
                    INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
                    SELECT @etl_imp_id, @client_id, 'etl_hst_load_ca_star()', 'b', v_etl_rpt_flag, v_etl_bm_build_flag;

                    call etl_hst_load_ca_star();
                    ...
                    Insert into tmp_test_type(test_type_moniker)
                         select distinct tt.moniker
                           ...
                           join c_ayp_test_type tt
                             on sub.ayp_test_type_id = tt.ayp_test_type_id
                          where sub.ayp_subject_code in ('cstHistory','cstMath','caeocAlgebraI',
									                                       'caeocBiology','capaMath1','capaELAI',
                                                         'cmaELA','cmaMath','cmaScience' )                      
                    ;                    
                    
                    
                    SET v_etl_rpt_flag = 1;
                    SET @sql_ca_star_log := '';
                    SET @sql_ca_star_log := CONCAT('call ', @db, '.imp_set_upload_file_status (\'', 'etl_hst_load_ca_star', '\',', '\'', 'P', '\',', '\'', 'ETL Load Successful', '\')');
                        prepare sql_ca_star_log from @sql_ca_star_log;
                                
                        execute sql_ca_star_log;
                            
                        deallocate prepare sql_ca_star_log;   
                    INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
                    SELECT @etl_imp_id, @client_id, 'etl_hst_load_ca_star()', 'c', v_etl_rpt_flag, v_etl_bm_build_flag;
                SELECT 'New ca star record' AS Uploader_ca_star, convert_tz(now(), 'UTC', 'US/Eastern') AS End_Time;
            ELSE SELECT 'No new ca star record' AS Uploader_ca_star;
        END IF;
        ...
        INSERT tmp.etl_imp_log (etl_imp_id, client_id, action, time_code, etl_rpt_flag, etl_bm_build_flag)
        SELECT @etl_imp_id, @client_id, 'etl_hst_ca', 'c', v_etl_rpt_flag, v_etl_bm_build_flag;
