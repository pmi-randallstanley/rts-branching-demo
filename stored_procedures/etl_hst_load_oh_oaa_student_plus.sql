...
drop procedure if exists etl_hst_load_oh_oaa_student_plus//

create definer=`dbadmin`@`localhost` procedure etl_hst_load_oh_oaa_student_plus()
contains sql
...
proc: begin 
   ...
   if v_view_exists > 0 then
        ...
        drop table if exists `tmp_oaa_subject_xref`;
        drop table if exists `tmp_oaa_strand_xref`;
        drop table if exists `tmp_oaa_stu_admin`;
        ...
        CREATE TABLE `tmp_oaa_subject_xref` (
          ...
          UNIQUE KEY `uq_tmp_oaa_subject_xref` (`client_ayp_subject_code`)
        ) ENGINE=InnoDB DEFAULT CHARSET=latin1
        ;

        CREATE TABLE `tmp_oaa_strand_xref` (
          `ayp_subject_id` int(10) NOT NULL,
        ...
        CREATE TABLE `tmp_oaa_stu_admin` (
        ...
        where   tt.moniker = 'oaa'
        ;

        # Strand Xref
        insert  tmp_oaa_strand_xref (
                  ayp_subject_id
                  ,ayp_strand_id
       ...
       join    tmp_oaa_subject_xref as sub
       ...
       join    tmp_oaa_subject_xref as sub2
       ...
       join    tmp_oaa_subject_xref as sub3
       ...
       ;
        ...
        if v_backfill_needed > 0 then
            ...
            from    tmp_oaa_stu_admin as sadmin
        ...
        select  tsa.ayp_subject_id
                ...
                ,now()
        from tmp_oaa_stu_admin as tsa
        join  tmp_oaa_strand_xref as hsl
        ...
        select  count(*)
        into    v_delete_count
        from    tmp_oaa_stu_admin as tmp1
