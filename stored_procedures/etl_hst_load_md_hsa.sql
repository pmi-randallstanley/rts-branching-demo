...
$HeadURL: http://atlanta-web.performancematters.com:8099/svn/pminternal/Data/Redwood/Core/stored_procedures/etl_hst_load_md_hsa.sql $
...
proc: begin 
    ...
    set v_ods_table = 'pmi_ods_hsa';
    set v_ods_view = 'v_pmi_ods_hsa';
    ...
    if v_view_exists > 0 then
        ...
        drop table if exists `tmp_hsa_subject_list`;
        drop table if exists `tmp_hsa_strand_list`;
        drop table if exists `tmp_stu_admin`;
        drop table if exists `tmp_hsa_test_date`;
        drop table if exists `tmp_student_year_backfill`;
        drop table if exists `tmp_school`;

        
        CREATE TABLE `tmp_hsa_test_date` (
          `test_mo_year` varchar(6) NOT NULL,
          `test_date` date NOT NULL,
          PRIMARY KEY  (`test_mo_year`),
          UNIQUE KEY `tmp_hsa_test_year_date` (`test_date`)
        ) ENGINE=InnoDB DEFAULT CHARSET=latin1
        ;

        CREATE TABLE `tmp_hsa_subject_list` (
        ...
        CREATE TABLE `tmp_hsa_strand_list` (
          `ayp_subject_id` int(10) NOT NULL, 
        ...
        CREATE TABLE `tmp_stu_admin` (
         ...
         Primary KEY (`row_num`),
         UNIQUE KEY `uq_tmp_stu_admin` (`student_code`, `client_ayp_subject_code`,`test_mo_year`),
        ...
        insert into tmp_hsa_test_date (
                    test_mo_year
                    ,test_date
        )
        select      test_mo_year
                   ,str_to_date(concat(left(ods.test_mo_year, 2),'/15/',substring(ods.test_mo_year,3,4)), '%m/%d/%Y')
        from v_pmi_ods_hsa ods
        group by    test_mo_year
                    ,str_to_date(concat(left(ods.test_mo_year, 2),'/15/',substring(ods.test_mo_year,3,4)), '%m/%d/%Y')
        ;

        insert  tmp_hsa_subject_list (
            ayp_subject_id
        ...
        where tab.target_table_name = 'pmi_ods_hsa'
        ;
        ...
        join    tmp_hsa_test_date as tmp
                on      ods.test_mo_year = tmp.test_mo_year
        join    c_student as s
                on      s.student_state_code = ods.pupil_number
        join    c_school_year as sty
                on     tmp.test_date between sty.begin_date and sty.end_date
        join    tmp_hsa_subject_list as sub
                on      ods.content_area = sub.client_ayp_subject_code
        ...
        join    tmp_hsa_test_date as tmp
                on       ods.test_mo_year = tmp.test_mo_year
        join    c_student as s
                on      s.fid_code = ods.pupil_number
        join    c_school_year as sty
                on     tmp.test_date between sty.begin_date and sty.end_date
        join    tmp_hsa_subject_list as sub
                on      ods.content_area = sub.client_ayp_subject_code
