...
$HeadURL: http://atlanta-web.performancematters.com:8099/svn/pminternal/Data/Redwood/Core/stored_procedures/etl_pm_natl_test_scores_act.sql $
$Id: etl_pm_natl_test_scores_act.sql 7435 2009-07-24 13:58:49Z randall.stanley $ 
 */
...
DROP PROCEDURE IF EXISTS etl_pm_natl_test_scores_act//

CREATE definer=`dbadmin`@`localhost` procedure etl_pm_natl_test_scores_act()
CONTAINS SQL
...
PROC: BEGIN
    ...
    and     t.table_name = 'v_pmi_ods_act';

    if @view_exists > 0 then
    
         set @actTestDateFormatMask := pmi_f_get_etl_setting('actTestDateFormatMask');

        if @actTestDateFormatMask is not null then
            set v_date_format_mask = @actTestDateFormatMask;
        end if;
    ...
    SET @sql_scan_log := CONCAT('call ', @db_name_ods, '.imp_set_upload_file_status (\'', 'pmi_ods_act', '\',', '\'', 'P', '\',', '\'', 'ETL Load Successful', '\')');
