...
drop procedure if exists etl_hst_load_ga_ghsgt_ss_qcc_gps//


create definer=`dbadmin`@`localhost` procedure etl_hst_load_ga_ghsgt_ss_qcc_gps()
contains sql
...
proc: begin 
    ...
    if v_view_exists > 0 then
        ...
        where sub.ayp_subject_code IN('ghsgtSocialStudies','ghsgtGPSSocStudies')
        ;  
        ...
        select  ods.row_num
            ,ods.stuid
            ,case when ods.socver = 'T' then 'Q' else ods.socver end socver 
            ,ods.testmo
            ,ods.testyr
            ,s.student_id
            ,case when sub.ayp_subject_id is null then 1 else sub.ayp_subject_id  end ayp_sub
            ,tmp.test_month
            ,sty.school_year_id
            ,ods.grade
            ,ods.schcode
        ...
        left join    tmp_ghsgt_subject_list as sub
                on      ods.socver = sub.client_ayp_subject_code
        left join   c_student_year as syr
                on syr.student_id = s.student_id
                and syr.school_year_id = sty.school_year_id
        where ods.socss REGEXP '^[0-9]' > 0
        union all
        select  ods.row_num
            ,ods.stuid
            ,case when ods.socver = 'T' then 'Q' else ods.socver end socver
        ...
        ###########################################
        #version code of T loaded as QCC
        ###########################################
        
        update tmp_stu_admin a
        join tmp_ghsgt_subject_list sub
            on   a.client_ayp_subject_code = sub.client_ayp_subject_code
            and  sub.ayp_subject_code = 'ghsgtSocialStudies'
        set a.ayp_subject_id = sub.ayp_subject_id
        where  a.client_ayp_subject_code = 'Q' and a.ayp_subject_id = 1 ;
