...
drop procedure if exists etl_hst_load_al_ARMT //

create definer=`dbadmin`@`localhost` procedure etl_hst_load_al_ARMT()
...
proc: begin 
    ...
    set v_ods_table = 'pmi_ods_al_armt_math_reading';
    set v_ods_view = 'v_pmi_ods_al_armt_math_reading';
    ...
    if v_view_exists > 0  and v_strand_grade_table_exists > 0 then
            ...
            drop table if exists  `tmp_armt_date_conversion`;
            ...
            create table `tmp_armt_date_conversion` (
          `ods_test_date` varchar(10) not null,
          `test_month` tinyint(2) not null,
          `test_date` date not null,
          unique key `uq_tmp_armt_date_conversion` (`test_date`)
          ) engine=innodb default charset=latin1
          ;
            ...
            create table `tmp_strand_list` (
                ...
                `ayp_strand_column_ss` varchar(50) not null,
                `begin_grade_sequence` varchar(15) default null,
                `end_grade_sequence` varchar(15),
                `begin_school_year` varchar(15),
                `end_school_year` varchar(15),
                PRIMARY KEY  (`ayp_subject_id`,`ayp_strand_id`,`begin_school_year`,`end_school_year`,`begin_grade_sequence`,`end_grade_sequence`)#(`ayp_subject_id`,`ayp_strand_id`,`grade_code`)
            ) ENGINE=InnoDB DEFAULT CHARSET=latin1
            ;
        ...
        insert into tmp_armt_date_conversion (
            ods_test_date
            ,test_month
            ,test_date
        )
        ...
        from    v_pmi_ods_al_armt_math_reading as ods
        ...
        ;
            ...
            select  ayps.ayp_subject_id
                    ,ayp_strand_id
                    ,min(itc.moniker) as ayp_strand_column_ss
                    ,begin_grade_sequence
                    ,end_grade_sequence
            ...
            join v_imp_table_column as itc 
                    on  ayps.table_id = itc.table_id
                    and ayps.score_column_id = itc.column_id
            ...
            where tab.target_table_name = ''', v_ods_table,'''
                  #and ayps.begin_grade_sequence = 0 
            ...
            select  ods.row_num
                    ...
                    ,case when sty.school_year_id is null then 1
                     when sty.school_year_id is not null and gl.grade_code = 'unassigned' then 1
                     end as backfill_needed_flag
            from    v_pmi_ods_al_armt_math_reading as ods
            join    c_student as s
                    on    s.student_code = ods.student_id
            join    tmp_armt_date_conversion as dte
            ...
            select  ods2.row_num
                    ...
                    when sty2.school_year_id is not null and gl.grade_code = 'unassigned' then 1
                     end as backfill_needed_flag
            from    v_pmi_ods_al_armt_math_reading as ods2
            join    c_student as s2
                    on    s2.student_state_code = ods2.student_id
            join    tmp_armt_date_conversion as dte2
            ...
            left join (c_student_year as sty2
                 inner join c_grade_level gl
                  on sty2.grade_level_id = gl.grade_level_id)
            ...
            select  ods3.row_num
                    ...
                    ,case when sty3.school_year_id is null then 1
                     when sty3.school_year_id is not null and gl.grade_code = 'unassigned' then 1
                     end as backfill_needed_flag
            from    v_pmi_ods_al_armt_math_reading as ods3
            join    c_student as s3
                    on    s3.fid_code = ods3.student_id
            join    tmp_armt_date_conversion as dte3
            ...
            left join (c_student_year as sty3
                 inner join c_grade_level gl
                  on sty3.grade_level_id = gl.grade_level_id) 
        ...
        into  v_ayp_subject_id, v_ayp_strand_id,  
                v_begin_school_year_id, v_end_school_year_id, v_begin_grade_sequence, v_end_grade_sequence, v_column_ss;
            ...
            SET @sql_text := concat(@sql_text,' insert c_ayp_strand_student (ayp_subject_id, ayp_strand_id, student_id, school_year_id '
                                             ,'                             ,month_id, ayp_score, last_user_id, create_timestamp) '
                                             ,' select  ', v_ayp_subject_id
                                             ,'         , ', v_ayp_strand_id
                                             ,'         ,sadmin.student_id '
                                             ,'         ,sadmin.school_year_id '
                                             ,'         ,sadmin.test_month '
                                             ,'         ,cast(ods.', v_column_ss, ' as decimal(9,3)) as ayp_score '
                                            ...
                                            # ,'join c_grade_level as grd '
                                             #,'     on gxref.pmi_grade_code = grd.grade_code'
                                            # ,' JOIN  tmp_imp_table_column_ayp_strand_grade as sm '
